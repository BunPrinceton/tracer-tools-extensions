...
Usage:
    ...
    python fast_validate_ids.py --input ids.txt --tracer-path "C:\\path\\to\\tracer_tools\\src"
"""
...
# ---- PATH SETUP (AUTO-DETECT + MANUAL FALLBACK) ----
# Tries to auto-locate `tracer_tools/src`. If that fails, use --tracer-path.

def find_tracer_tools(manual_path=None):
    candidates = []

    # If the user provided --tracer-path, check it first
    if manual_path:
        candidates.append(Path(manual_path))

    candidates += [
        # Same folder as script
        Path(__file__).parent / "tracer_tools" / "src",

        # One level up
        Path(__file__).parent.parent / "tracer_tools" / "src",

        # If script is inside "Tracer - Workspace"
        Path(__file__).parent / "Tracer - Workspace" / "tracer_tools" / "src",
        Path(__file__).parent.parent / "Tracer - Workspace" / "tracer_tools" / "src",
    ]
...
# Do a quick pre-parse just to grab --tracer-path before full argparse runs
_pre = argparse.ArgumentParser(add_help=False)
_pre.add_argument("--tracer-path", default=None)
_known, _rest = _pre.parse_known_args()

tracer_tools_path = find_tracer_tools(_known.tracer_path)

if tracer_tools_path:
    sys.path.insert(0, str(tracer_tools_path))
else:
    print()
    ...
    print()
    print("  OPTION A - Place the script in the right folder:")
    print("    Put this script inside (or next to) your Tracer - Workspace folder:")
    print("      Tracer - Workspace/")
    print("          tracer_tools/")
    print("              src/")
    print("          fast_validate_ids.py   <-- here")
    print()
    print("  OPTION B - Tell the script where tracer_tools is:")
    print('    python fast_validate_ids.py --input ids.txt --tracer-path "C:\\your\\path\\to\\tracer_tools\\src"')
    print()
    print("    To find the right path, locate the 'tracer_tools' folder on your")
    print("    computer and use the 'src' subfolder inside it.")
    print()
    sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        "--tracer-path", default=None,
        help="Path to tracer_tools/src (only needed if auto-detect fails)"
    )
